...
echo "Ensuring member-name map file exists for user/usergroup fields..."
sudo -H -u "${OPENCLAW_USER}" bash -c 'set -euo pipefail; mkdir -p "$HOME/.openclaw-main"; if [[ ! -f "$HOME/.openclaw-main/jiandaoyun-member-map.json" ]]; then printf "%s\n" "{}" > "$HOME/.openclaw-main/jiandaoyun-member-map.json"; fi; chmod 600 "$HOME/.openclaw-main/jiandaoyun-member-map.json"; ls -l "$HOME/.openclaw-main/jiandaoyun-member-map.json"'
...
servers.jiandaoyun.env.JIANDAOYUN_MEMBER_MAP_FILE = `${process.env.HOME}/.openclaw-main/jiandaoyun-member-map.json`;
servers.jiandaoyun.env.JIANDAOYUN_ROOT_DEPT_NO = servers.jiandaoyun.env.JIANDAOYUN_ROOT_DEPT_NO || "1";
...
servers.jiandaoyun.env.JIANDAOYUN_WEACT_CREATOR_FIELD = servers.jiandaoyun.env.JIANDAOYUN_WEACT_CREATOR_FIELD || "user_id";
...
echo "If a SenderId is not mapped, the plugin will try weact-cli contact +get-user, map unique identity fields, then resolve the WeACT identity through Jiandaoyun contacts."
echo "User/usergroup fields now resolve display names through Jiandaoyun contact lookup; duplicate names require jiandaoyun-member-map.json."
